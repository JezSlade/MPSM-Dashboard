<?php
/**
 * Alert System Setup - TEST VERSION (NO AUTH)
...
echo "=== MPSM Alert System Setup (TEST) ===\n\n";

// Menu
if ($action === 'menu') {
    echo "Available actions:\n\n";
    echo "1. Insert Alert Rules:\n";
    echo "   http://localhost:8000/test-setup-alerts.php?action=insert_rules\n\n";
    echo "2. Reprocess Panel Messages:\n";
    echo "   http://localhost:8000/test-setup-alerts.php?action=reprocess\n\n";
    echo "3. View Status:\n";
    echo "   http://localhost:8000/test-setup-alerts.php?action=status\n\n";
    exit;
}
...
if ($action === 'reprocess') {
    ...
    if ($activeRules === 0) {
        ...
        echo "Insert rules first: http://localhost:8000/test-setup-alerts.php?action=insert_rules\n";
        exit(1);
    }

    // Get processing limit from query param (default 100 for testing)
    ...
    if (!empty($severityCounts)) {
        ...
        echo "No active notifications found (this is normal if no devices meet thresholds).\n";
    }
