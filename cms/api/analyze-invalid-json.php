<?php
/**
 * Analyze Invalid JSON Errors (Web-accessible version)
 * Direct database query for diagnostic purposes
 */

require_once dirname(__DIR__, 2) . '/bootstrap.php';
require_once dirname(__DIR__) . '/functions.php';
requireAuth();

header('Content-Type: text/plain; charset=utf-8');

try {
    $pdo = getDatabase();
    $table = DB_PREFIX . 'panel_callback_debug';

    echo "=== INVALID JSON PAYLOAD ANALYSIS ===\n\n";

    // Count total invalid JSON errors
    $stmt = $pdo->query("
        SELECT COUNT(*) as total
        FROM {$table}
        WHERE status = 'ERROR' AND message = 'Invalid JSON payload'
    ");
    $errorCount = $stmt->fetchColumn();

    echo "Total Invalid JSON Payload Errors: {$errorCount}\n\n";

    // Get sample of invalid payloads for analysis
    $stmt = $pdo->query("
        SELECT id, timestamp, raw_body, unique_source, ip_address
        FROM {$table}
        WHERE status = 'ERROR' AND message = 'Invalid JSON payload'
        ORDER BY timestamp DESC
        LIMIT 50
    ");

    echo "Sample Invalid Payloads (Most Recent 50):\n";
    echo str_repeat('=', 120) . "\n\n";

    $validJsonCount = 0;
    $actuallyInvalidCount = 0;
    $emptyCount = 0;
    $nullJsonCount = 0;
    $nonArrayJsonCount = 0;
    $patterns = [];

    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        $rawBody = trim($row['raw_body'] ?? '');

        if ($rawBody === '') {
            $emptyCount++;
            continue;
        }

        // Check if it's actually valid JSON
        $decoded = json_decode($rawBody, true);
        $jsonError = json_last_error();

        if ($jsonError === JSON_ERROR_NONE) {
            // JSON is VALID, but let's see what it decoded to
            if ($decoded === null) {
                $nullJsonCount++;
                echo "ID {$row['id']} - Time: {$row['timestamp']}\n";
                echo "⚠️  VALID JSON 'null' value (rejected because not an array)\n";
                echo "Source: {$row['unique_source']} | IP: {$row['ip_address']}\n";
                echo "Body: {$rawBody}\n";
                echo str_repeat('-', 120) . "\n\n";
            } elseif (!is_array($decoded)) {
                $nonArrayJsonCount++;
                echo "ID {$row['id']} - Time: {$row['timestamp']}\n";
                echo "⚠️  VALID JSON but NOT an array (type: " . gettype($decoded) . ")\n";
                echo "Source: {$row['unique_source']} | IP: {$row['ip_address']}\n";
                echo "Body: {$rawBody}\n";
                echo "Decoded value: " . var_export($decoded, true) . "\n";
                echo str_repeat('-', 120) . "\n\n";
            } else {
                $validJsonCount++;
                echo "ID {$row['id']} - Time: {$row['timestamp']}\n";
                echo "✓ VALID JSON array (This should NOT have been rejected!)\n";
                echo "Source: {$row['unique_source']} | IP: {$row['ip_address']}\n";
                echo "Sample data:\n";
                echo substr(json_encode($decoded, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES), 0, 500) . "...\n";
                echo str_repeat('-', 120) . "\n\n";
            }
        } else {
            $actuallyInvalidCount++;

            // Track patterns
            $first50 = substr($rawBody, 0, 50);
            if (!isset($patterns[$first50])) {
                $patterns[$first50] = 0;
            }
            $patterns[$first50]++;

            // Only show first 10 actually invalid ones
            if ($actuallyInvalidCount <= 10) {
                echo "ID {$row['id']} - Time: {$row['timestamp']}\n";
                echo "✗ Actually Invalid JSON\n";
                echo "Error: " . json_last_error_msg() . "\n";
                echo "Source: {$row['unique_source']} | IP: {$row['ip_address']}\n";
                echo "Body (first 200 chars): " . substr($rawBody, 0, 200) . "...\n";
                echo str_repeat('-', 120) . "\n\n";
            }
        }
    }

    echo "\n" . str_repeat('=', 120) . "\n";
    echo "SUMMARY:\n";
    echo "  • Valid JSON arrays (incorrectly rejected): {$validJsonCount}\n";
    echo "  • Valid JSON 'null' value (correctly rejected - not array): {$nullJsonCount}\n";
    echo "  • Valid JSON non-array (correctly rejected - not array): {$nonArrayJsonCount}\n";
    echo "  • Actually invalid JSON: {$actuallyInvalidCount}\n";
    echo "  • Empty/whitespace bodies: {$emptyCount}\n";
    echo "  • Total analyzed: " . ($validJsonCount + $nullJsonCount + $nonArrayJsonCount + $actuallyInvalidCount + $emptyCount) . "\n";
    echo str_repeat('=', 120) . "\n\n";

    if (!empty($patterns)) {
        echo "Common Invalid Patterns:\n";
        arsort($patterns);
        foreach (array_slice($patterns, 0, 5, true) as $pattern => $count) {
            echo "  [{$count}x] {$pattern}...\n";
        }
        echo "\n";
    }

    // Now count drill-down cache
    echo "\n=== DRILL-DOWN CACHE ANALYSIS ===\n\n";

    $drilldownTable = DB_PREFIX . 'cache_device_drilldown';
    $devicesTable = DB_PREFIX . 'cache_devices';

    $stmt = $pdo->query("SELECT COUNT(*) FROM {$drilldownTable}");
    $drilldownCount = $stmt->fetchColumn();

    $stmt = $pdo->query("SELECT COUNT(*) FROM {$devicesTable}");
    $devicesCount = $stmt->fetchColumn();

    echo "Total Devices in Cache: {$devicesCount}\n";
    echo "Total Devices with Drill-Down Cache: {$drilldownCount}\n";

    $coverage = $devicesCount > 0 ? round(($drilldownCount / $devicesCount) * 100, 2) : 0;
    echo "Drill-Down Coverage: {$coverage}%\n\n";

    // Show some stats about drill-down cache
    $stmt = $pdo->query("
        SELECT
            COUNT(*) as total,
            SUM(CASE WHEN has_alerts = 1 THEN 1 ELSE 0 END) as with_alerts,
            SUM(CASE WHEN has_supplies = 1 THEN 1 ELSE 0 END) as with_supplies,
            MIN(cached_at) as oldest,
            MAX(cached_at) as newest
        FROM {$drilldownTable}
    ");
    $stats = $stmt->fetch(PDO::FETCH_ASSOC);

    echo "Drill-Down Cache Statistics:\n";
    echo "  • With Alerts: {$stats['with_alerts']} (" . ($stats['total'] > 0 ? round(($stats['with_alerts'] / $stats['total']) * 100, 1) : 0) . "%)\n";
    echo "  • With Supplies: {$stats['with_supplies']} (" . ($stats['total'] > 0 ? round(($stats['with_supplies'] / $stats['total']) * 100, 1) : 0) . "%)\n";
    echo "  • Oldest Entry: {$stats['oldest']}\n";
    echo "  • Newest Entry: {$stats['newest']}\n\n";

    // Show recent drill-down cache entries
    $stmt = $pdo->query("
        SELECT serial_number, cached_at, has_alerts, has_supplies
        FROM {$drilldownTable}
        ORDER BY cached_at DESC
        LIMIT 15
    ");

    echo "Recent Drill-Down Cache Entries (Last 15):\n";
    echo str_repeat('=', 100) . "\n";
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        echo str_pad($row['serial_number'], 25) . " | ";
        echo $row['cached_at'] . " | ";
        echo "Alerts: " . ($row['has_alerts'] ? 'Yes' : 'No ') . " | ";
        echo "Supplies: " . ($row['has_supplies'] ? 'Yes' : 'No ') . "\n";
    }

    echo "\n";

} catch (Exception $e) {
    echo "ERROR: " . $e->getMessage() . "\n";
    echo $e->getTraceAsString() . "\n";
}
