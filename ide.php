<?php
/**
 * Refactored IDE - Enhanced Widgets IDE
 * Implemented:
 *  - Alphabetical sorting of files
 *  - Timestamped backup files for versioning
 *  - Fully functional breadcrumb navigation
 *  - "New Widget" button to auto-generate widget template files
 */
...
function newWidgetPrompt(){const name=prompt('Enter new widget name:');if(!name)return;fetch('',{method:'POST',body:new URLSearchParams({action:'newWidget',name})}).then(r=>r.json()).then(d=>{if(d.success){alert('Widget created');loadSidebar()}else alert('Widget already exists or error')})}
loadSidebar();
</script></body></html>
