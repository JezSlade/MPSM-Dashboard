 ...
 * debug.php
 *
 * A “crazy robust” diagnostic script for the MPSM Dashboard skeleton (no SCSS).
 * This version only checks for the compiled CSS (`assets/css/styles.css`), not any SCSS sources.
 *
 * Usage:
 *   1. Place this file and debug_checks.json (above) in /public/mpsm/
 ...
 *
 * What it does:
 *   A. Validates files/folders per debug_checks.json (no SCSS checks).
 *   B. Parses index.php for <link rel="stylesheet"> tags and checks each CSS reference.
 ...
 *   E. Outputs an HTML report plus machine-readable JSON and a “Copy JSON” button.
 */
...
if (!file_exists($indexPath) || !is_readable($indexPath)) {
  ...
  if (empty($matches[1])) {
    addCheck($results, $section, 'CSS Links', 'WARN',
      ...
      'Ensure index.php references at least one CSS file.'
    );
  } else {
    foreach ($matches[1] as $href) {
      $hrefClean = explode('?', $href, 2)[0];
      // If external URL, assume PASS
        ...
        if (is_readable($cssPath)) {
          ...
          if ($size > 0) {
            ...
            addCheck($results, $section, $hrefClean, 'FAIL',
              ...
              'Verify that styles.css was generated properly.'
            );
          }
        } else {
          addCheck($results, $section, $hrefClean, 'FAIL',
