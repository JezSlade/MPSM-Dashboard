...
// Adds a per-table settings icon (smaller) to choose visible columns & page size.
// Defaults to all columns returned by the API.
// -------------------------------------------------------------------
...
function renderDataTable(array $data, array $options = []): void {
    ...
    $allCols = array_combine($allKeys, $allKeys);

    // 2) Settings + unique IDs
    $tableId     = uniqid('dt_');
    ...
    // 3) JSON-encode data for JS
    $jsData = array_map(function($row) {
        return array_map(fn($c)=> is_array($c)? json_encode($c): $c, (array)$row);
    }, $data);
    ...
    <?php if ($searchable): ?>
      <input type="text" id="<?= $tableId ?>_search"
             ...
             class="w-1/2 text-sm bg-gray-700 text-white border border-gray-600 
                    rounded-md py-1 px-3 focus:outline-none focus:ring-2 focus:ring-cyan-500" />
    <?php endif; ?>
...
(function(){
  ...
  try {
    ...
    if (Array.isArray(s.visibleCols)) {
      colCheckboxes.forEach(cb => cb.checked = s.visibleCols.includes(cb.dataset.colKey));
    }
  } catch {}
  ...
  function applySettings() {
    ...
    localStorage.setItem('<?= $settingsKey ?>',
      JSON.stringify({rpp, visibleCols: visible})
    );
  ...
  function renderTable() {
    // Sort & paginate
    filtered.sort((a,b)=>{
      const v1=(a[sortKey]||'').toLowerCase(), v2=(b[sortKey]||'').toLowerCase();
    ...
    tblBody.innerHTML = rows.map((row,i)=> {
      ...
      const cells = columns.map(key=>
        `<td class="px-4 py-2 text-gray-100">${row[key]||''}</td>`
      );
      return `<tr class="${cls}">${cells.join('')}</tr>`;
    }).join('') || `<tr><td colspan="${columns.length}" class="px-4 py-2 text-center text-gray-300">
        No data
      </td></tr>`;
  ...
  function renderPager(){
    ...
    pager.querySelectorAll('button').forEach(btn=>
      btn.addEventListener('click',()=> {
        currentPage=+btn.dataset.page; renderTable();
      })
    );
  }
