...
try {
    $file = $repoRoot . '/cms/api/get-duplicate-ips.php';
...
try {
    ...
    include $repoRoot . '/cms/api/device-age-report.php';
    $output = ob_get_clean();
...
try {
    ...
    include $repoRoot . '/cms/api/get-dealer-summary.php';
    $output = ob_get_clean();
...
// Save report
file_put_contents(dirname(__DIR__) . '/reports/test_apis_direct_report.json', json_encode([
    'timestamp' => $now,
    'results' => $results
], JSON_PRETTY_PRINT));

echo "Report saved to: tests/reports/test_apis_direct_report.json\n";

$failed = array_filter($results, static function (array $result): bool {
    return ($result['status'] ?? 'ERROR') !== 'PASS';
});
