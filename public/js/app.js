 ...
 * ------------------------------------------------------
 * Implements:
 * 1) OAuth2 token fetch + auto-refresh → then loadCustomers()
 * 2) Single searchable Customer dropdown
 * 3) Developer-only debug panel toggle
 * 4) Sidebar with per-role icons from the `icons` map
...
(function(){
  ...
  const icons = {
    Developer:  '<svg class="icon" viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="2"><path d="M16 18l6-6-6-6M8 6l-6 6 6 6"/></svg>',
    Admin:      '<svg class="icon" viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="2"><path d="M9 17v2a2 2 0 002 2h2a2 2 0 002-2v-2M4 9h12"/></svg>',
    Dealer:     '<svg class="icon" viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 10h4l3-6 4 12h4"/></svg>',
    Service:    '<svg class="icon" viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="2"><circle cx="10" cy="10" r="3"/><path d="M2 10a8 8 0 0116 0"/></svg>',
    Sales:      '<svg class="icon" viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 15l6-6 4 4 4-8"/></svg>',
    Accounting: '<svg class="icon" viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="14" height="4"/><rect x="3" y="9" width="14" height="4"/><rect x="3" y="15" width="14" height="2"/></svg>',
    Guest:      '<svg class="icon" viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="2"><circle cx="10" cy="6" r="3"/><path d="M2 18a8 8 0 0116 0"/></svg>'
  };
  ...
  // 2) Load customers into datalist
  ...
  document.addEventListener('DOMContentLoaded', () => {
    // Start token + customer load
    fetchToken();

    // 3) Build sidebar with role icons
    ...
    // Initial render
    const firstRole = Object.keys(mappings)[0];
