...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        if (isset($_POST['update_env'])) {
            ...
            echo "<p class='text-green-500 p-2'>.env updated successfully! 🔧</p>";
            ...
            echo "<p class='text-green-500 p-2'>CSS updated successfully! 🎨</p>";
        } elseif (isset($_POST['clear_cache'])) {
            session_unset();
            session_destroy();
            session_start();
            $_SESSION['user_id'] = 1; // Re-authenticate as admin
            echo "<p class='text-green-500 p-2'>Session cache cleared! 🗑️</p>";
...
<h2 class="text-2xl text-teal-custom mb-4">DevTools ⚙️</h2>
...
<div class="space-y-6">
    <div class="glass p-4 border border-gray-800 rounded">
        ...
        <form method="POST" class="mt-4 space-y-4">
            ...
            <button type="submit" name="update_env" class="bg-gray-800 text-teal-custom p-2 rounded border border-gray-700 hover:bg-gray-700">💾 Save .env</button>
        </form>
    </div>
    ...
    <div class="glass p-4 border border-gray-800 rounded">
        ...
        <form method="POST" class="mt-4 space-y-4">
            ...
            <button type="submit" name="update_css" class="bg-gray-800 text-teal-custom p-2 rounded border border-gray-700 hover:bg-gray-700">💾 Save CSS</button>
        </form>
    </div>
    ...
    <div class="glass p-4 border border-gray-800 rounded">
        ...
        <form method="POST" class="mt-4 space-y-4">
            <div>
                ...
                <button type="submit" name="toggle_debug" class="bg-gray-800 text-teal-custom p-2 rounded border border-gray-700 hover:bg-gray-700">🔍 Toggle Debug</button>
            </div>
            <form method="POST" class="mt-2">
                <button type="submit" name="clear_cache" class="bg-gray-800 text-teal-custom p-2 rounded border border-gray-700 hover:bg-gray-700">🗑️ Clear Session Cache</button>
            </form>
        </form>
    </div>
