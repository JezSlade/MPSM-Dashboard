...
The historical PowerShell diagnostics helper is not present in the current active repository. Use the CMS UI and `cms/api/get-panel-messages.php` for live checks, or recreate this as a portable Python/shell helper if CLI diagnostics are needed again.
...
| Portable panel-message diagnostics helper | CLI diagnostics | Not currently present; recreate in Python/shell if needed |
