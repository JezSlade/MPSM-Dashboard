...
> Historical status note from 2025-11. It does not describe the current cache workflow. Current cache operations use `cms/api/refresh-cache-chunked.php` with `action=start`, repeated `action=process`, `action=status`, and guarded `action=cutover`.
...
- [cms/api/cache-status-report.php](../../cms/api/cache-status-report.php) - Cache health checker

### Documentation
- [CACHE_SYSTEM_AUDIT.md](../reports/CACHE_SYSTEM_AUDIT.md) - Comprehensive technical audit
...
### Scripts
- [start-cache-refresh.ps1](../../scripts/powershell/start-cache-refresh.ps1) - Manual cache refresh script
- [cache-population-loop.ps1](../../scripts/powershell/cache-population-loop.ps1) - Continuous refresh loop (advanced)
