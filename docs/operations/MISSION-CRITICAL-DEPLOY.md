...
> Historical runbook. This document records a prior notification-population deploy plan and is not the current deployment procedure. For current production deployment use `docs/operations/DEPLOY-INSTRUCTIONS.md`, `scripts/ftp_backup.py`, `scripts/ftp_deploy.py`, and `scripts/live_smoke.py`.
