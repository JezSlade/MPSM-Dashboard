...
?>
<main class="flex-1 overflow-auto p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 neon-glass-viewport">
  <?php foreach ($visible as $id):
    $file = __DIR__ . "/../cards/card_{$id}.php";
    if (file_exists($file)) {
      include $file;
    } else {
      $title = htmlspecialchars($cardsConfig[$id]??$id);
      echo "<div class='panel text-red-500 tooltip' title='Card missing'>{$title}</div>";
    }
  endforeach; ?>
</main>
