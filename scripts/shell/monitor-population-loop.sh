...
if [ -n "${MPSM_BASIC_AUTH_USER:-}" ] && [ -n "${MPSM_BASIC_AUTH_PASSWORD:-}" ]; then
    AUTH_ARGS=(--user "${MPSM_BASIC_AUTH_USER}:${MPSM_BASIC_AUTH_PASSWORD}")
fi
...
while [ $ATTEMPT -lt $MAX_ATTEMPTS ]; do
    ...
    # Get current count
    COUNT_JSON=$(curl -s --max-time 10 "https://mpsm.resolutionsbydesign.us/cms/api/quick-count.php" "${AUTH_ARGS[@]}" 2>&1)
    ...
    if [ -f /tmp/last_device_count ]; then
        ...
        if [ "$DEVICES" -eq "$LAST_COUNT" ]; then
            ...
            CHUNK_RESULT=$(curl -s --max-time 300 "https://mpsm.resolutionsbydesign.us/cms/api/populate-chunked.php" "${AUTH_ARGS[@]}" 2>&1)
